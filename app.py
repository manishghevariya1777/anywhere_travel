...
def main():
    ...
    if submit_button:
        ...
        # Validate inputs
        is_valid, error_msg = validate_inputs(origin, destination, 7, preferences)
        if not is_valid:
            st.error(error_msg)
            return
        ...
        with st.spinner("🔍 Creating your personalized travel plan..."):
            ...
            weather_data = get_weather_forecast(destination, days=7)
            if weather_data:
                weather_info = "\n\n### Weather Forecast\n"
            ...
            currency_symbol = get_currency_symbol(destination)
            if currency_symbol:
                preferences["currency"] = currency_symbol
            
            # Get cost estimation
            cost_estimation = estimate_total_cost(origin, destination, 7, "Mid-Range")
            cost_info = "\n\n### Cost Estimation\n"
            cost_info += f"- Flight Cost: ${cost_estimation['flight_cost']:.2f}\n"
            cost_info += f"- Hotel Cost: ${cost_estimation['hotel_cost']:.2f}\n"
            cost_info += f"- Daily Expenses: ${cost_estimation['daily_expenses']:.2f}\n"
            cost_info += f"- Daily Budget: ${cost_estimation['daily_budget']:.2f}\n"
            cost_info += f"- Total Cost: ${cost_estimation['total_cost']:.2f}\n"
            search_data += cost_info
            
            # Planning phase
            result = generate_plan(destination, 7, preferences, search_data)
